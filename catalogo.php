...
<head>
  ...
  <style>
    .plant-card-sku { font-family: var(--font-heading); font-size: 0.7rem; font-weight: 700; letter-spacing: 0.07em; color: var(--muted); margin-top: auto; padding-top: 0.375rem; }
    ...
    @media (max-width: 1024px) {
      ...
      .filter-mobile-toggle:hover { background: var(--green-100); }
    ...
    .pagination button:hover:not(:disabled):not(.is-active) { background: var(--green-100); border-color: var(--green-700); }
    .pagination button.is-active { background: var(--green-700); color: var(--white); border-color: var(--green-700); cursor: default; }
    .pagination button:disabled { opacity: 0.4; cursor: not-allowed; }
    .pagination .pagination-ellipsis {
      min-width: 1.5rem; text-align: center; color: var(--muted);
...
<body>
  ...
  <main>
    ...
    <!-- PAGE HERO -->
    <section class="page-hero" style="padding:3.5rem 0 2.5rem;" aria-label="Catálogo de plantas">
      <div class="container">
        <p class="eyebrow"><span class="material-symbols-outlined">local_florist</span>Nuestras plantas</p>
        <h1 style="font-size:clamp(1.75rem,4vw,2.75rem);">Catálogo</h1>
        <p style="font-size:1rem;">Explora nuestra selección y consulta disponibilidad directamente.</p>
      </div>
    </section>

    <!-- CATALOG -->
    <section class="section-sm" aria-labelledby="catalog-heading">
      <div class="container">
        ...
        <div class="catalog-layout">
          ...
          <aside aria-label="Filtros de búsqueda">
            <button class="filter-mobile-toggle" id="filterToggle" aria-expanded="false" aria-controls="filterPanel">
              <span class="material-symbols-outlined" style="font-size:1.125rem;">filter_list</span>
              Filtros
            </button>
            <div class="filter-panel" id="filterPanel" role="search" aria-label="Panel de filtros">
              <h3>Filtros</h3>
              ...
              <div class="filter-group">
                ...
                <div class="filter-options" role="group" aria-labelledby="cuidado-label">
                  ...
                  <label class="filter-option"><input type="checkbox" name="cuidado" value="intermedia"> Intermedio</label>
              ...
              <button class="filter-reset" id="resetFilters" type="button">Limpiar filtros</button>
            </div>
          </aside>
  ...
  <footer class="site-footer">
    <div class="container">
      ...
      <div class="footer-bottom">
        ...
        <span>Plantas ornamentales desde 1992</span>
      </div>
    </div>
  </footer>
  ...
  <script type="module">
    ...
    (async function () {
      const dispCls = { disponible: 'badge-available', 'bajo pedido': 'badge-order', agotado: 'badge-sold' };
        ...
        return `
          <a href="${href}" class="card plant-card" aria-label="Ver detalle de ${p.nombre}">
            ...
            <div class="plant-card-body">
              ...
              <div class="plant-card-name">${p.nombre}</div>
              <div class="plant-card-sci">${p.nombreCientifico}</div>
              ${p.sku ? `<div class="plant-card-sku">${p.sku}</div>` : ''}
              <div class="plant-card-meta">
                <span class="plant-card-meta-item"><span class="material-symbols-outlined">wb_sunny</span>${p.luz}</span>
                <span class="plant-card-meta-item"><span class="material-symbols-outlined">water_drop</span>${p.riego}</span>
                <span class="plant-card-meta-item"><span class="material-symbols-outlined">eco</span>${p.cuidado}</span>
              </div>
            </div>
          </a>`;
      }
      ...
      function applyFilters() {
        ...
        currentList = plantas.filter(p => {
          ...
          if (filters.disponibilidad !== 'todas' && p.disponibilidad !== filters.disponibilidad) return false;
      ...
      function render() {
        ...
        if (list.length === 0) {
          ...
          grid.innerHTML = `<div class="empty-state" style="grid-column:1/-1;"><span class="material-symbols-outlined">search_off</span><h3>Sin resultados</h3><p>¿No encuentras lo que buscas? <a href="https://wa.me/${WHATSAPP_NUMBER}?text=${waMsg}" target="_blank" rel="noopener noreferrer" style="color:var(--green-700);font-weight:700;text-decoration:underline;">¡Escríbenos!</a></p></div>`;
          pagEl.hidden = true;
          pagEl.innerHTML = '';
          return;
        }
      ...
      pagEl.addEventListener('click', e => {
        ...
        const top = (heading?.getBoundingClientRect().top || 0) + window.scrollY - 80;
